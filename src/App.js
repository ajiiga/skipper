...
function App() {
  ...
  if (authStore.isInitialisation) return <Preloader />;
  return (
    <BrowserRouter>
      <div className={s.app}>
        ...
        <div className={s.container}>
          <Switch>
            ...
            {authStore.isAuth &&
              ...
              AuthService.urls
                .filter((page) => page.path === "/mentor_registration")
                .map((route) => (
                  <Route
                    key={route.path}
                    exact={route.exact}
            ...
            {authStore.isAuth &&
              PrivateProfileService.urls.map((route) => (
                <Route
                  key={route.path}
                  exact={route.exact}
                  path={route.path}
            ...
            {PublicProfilesService.map((route) => (
              <Route
                key={route.path}
                exact={route.exact}
                component={route.component}
            ...
            {authStore.isAuth &&
              authStore.user?.is_mentor &&
              MyClassesService.filter((x) => x.for_mentor).map((route) => (
                <Route
                  key={route.path}
                  exact={route.exact}
                  component={route.component}
            ...
            {authStore.isAuth &&
              MessagesService.urls.map((route) => (
                <Route
                  key={route.path}
                  exact={route.exact}
                  component={route.component}
