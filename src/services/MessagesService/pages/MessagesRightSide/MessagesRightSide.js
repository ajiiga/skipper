...
const MessagesRightSide = ({
        ...
        socket.current.on('message', (data) => {
            setChatInfo((chatInfo) => {
                let newMessages = [
                    ...
                    {
                        ...JSON.parse(data),
                        stringDateAndTime: messagesStore.refactorDate(
                            new Date().toString()
                        ),
                    },
                ];
    ...
    useEffect(() => {
        ...
        messagesStore.getCurrentChat(params.id).then((x) => {
            ...
            connect(x);
            if (
                chatList.filter((chats) => chats.ID === x.chat.ID).length === 0
            ) {
    ...
    return (
        <div className={s.container}>
            <MessagesRightSideTitle
                firstName={chatInfo.chat.FirstName}
                secondName={chatInfo.chat.SecondName}
                img={chatInfo.chat.ProfilePicture}
                id={chatInfo.chat.ID}
                isMentor={chatInfo.chat.IsMentor}
            />
            ...
            <Switch>
                ...
                <Route path={`/messages/${params.id}/stop-lesson`}>
                    <StopLessonModal
                        title={'Прекратить занятия'}
                        id={params.id}
                    />
                </Route>
                <Route path={`/messages/${params.id}/reject-lesson`}>
                    <StopLessonModal
                        title={'Отклонить занятия'}
                        id={params.id}
                    />
                </Route>
                ...
                <Route path={`/messages/${params.id}/termination-lesson`}>
                    <TerminationLessonModal
                        title={'Прекращение занятий'}
                        id={params.id}
                    />
                </Route>
                <Route path={`/messages/${params.id}/rejected-lesson`}>
                    <TerminationLessonModal
                        title={'Отклонение занятий'}
                        id={params.id}
                    />
                </Route>
                <Route path={`/messages/${params.id}/change-communication`}>
...
const MessagesNavigator = () => {
    ...
    let setReview = <img src={set_review} alt="" />;
