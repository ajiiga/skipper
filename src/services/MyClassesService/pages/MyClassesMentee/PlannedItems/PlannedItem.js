  ...
  return (
    <div className={s.container}>
      <div className={s.title_container}>
        ...
        <button className={s.right_title_btn_v2} onClick={() => cancelItem()}>
          Прекратить занятия
        </button>
      </div>
      ...
      <div className={s.schedule_container}>
        <div className={s.schedule}>
          {data.fixedTime.map((x, index) => {
            ...
            return (
              <div className={`${s.schedule_item} ${deltaClass}`}>
                <div>{x.name}</div>
                <div>{x.time}</div>
              </div>
            );
      ...
      <div className={s.communication_price_container}>
        <div
          className={`${s.communication_container} ${s.communication_container_mentee}`}
        >
